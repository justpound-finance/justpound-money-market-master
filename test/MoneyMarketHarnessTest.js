...
contract('MoneyMarketHarness', function([root, ...accounts]) {
  ...
  describe('supply', async () => {
    ...
    it('emits supply event on success', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.supplyBalances(customer, OMG._address).call();
      assert.equal(90, customerJustpoundBalance.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance.interestIndex);
    });
    ...
    it('allows supply of 0 even for customer with no token', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.supplyBalances(customer, OMG._address).call();
      assert.equal(0, customerJustpoundBalance.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance.interestIndex);
    });
  });
  ...
  describe('withdraw', async() => {
    ...
    it('emits SupplyWithdrawn event on success', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.supplyBalances(customer, OMG._address).call();
      assert.equal(99, customerJustpoundBalance.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance.interestIndex);
    });
    ...
    it('fails withdraw with zero oracle price', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.supplyBalances(customer, OMG._address).call();
      assert.equal(100, customerJustpoundBalance.principal);
    });
    ...
    it('special withdraw max value allows partial token withdrawal when customer has limited liquidity', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.supplyBalances(customer, OMG._address).call();
      assert.equal(65, customerJustpoundBalance.principal);
    });
  ...
  describe('repayBorrow', async () => {
    ...
    it('emits borrow repaid event on success', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.borrowBalances(customer, OMG._address).call();
      assert.equal(10, customerJustpoundBalance.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance.interestIndex);
    });
    ...
    it('handles special pay max value when borrow balance exceeds customer token balance', async () => {
      ...
      const customerJustpoundBalance = await moneyMarketHarness.methods.borrowBalances(customer, OMG._address).call();
      assert.equal(20, customerJustpoundBalance.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance.interestIndex);
    });
  });
  ...
  async function setupValidLiquidation(nonStandard=false) {
    ...
    // Give the other supplier some borrow asset and supply it to Justpound.
    ...
    // Give borrower some collateral and supply it to Justpound
  ...
  describe('borrow', async () => {
    ...
    it('succeeds, fails and succeeds', async () => {
      ...
      const customerJustpoundBalance0 = await moneyMarketHarness.methods.borrowBalances(customer, OMG._address).call();
      assert.equal(5, customerJustpoundBalance0.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance0.interestIndex);
      ...
      const customerJustpoundBalance1 = await moneyMarketHarness.methods.borrowBalances(customer, OMG._address).call();
      assert.equal(5, customerJustpoundBalance1.principal);
      assert.equal(initialInterestIndex, customerJustpoundBalance1.interestIndex);
      ...
      const customerJustpoundBalance2 = await moneyMarketHarness.methods.borrowBalances(customer, OMG._address).call();
      assert.equal(13, customerJustpoundBalance2.principal);
      assert.equal(2 * initialInterestIndex, customerJustpoundBalance2.interestIndex);
    });
  });
});